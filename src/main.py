...
def f(gene: Gene, x: float) -> float:
    return gene.a * (x**2 - gene.b * cos(gene.c * pi * x))
...
def main():
    while not gene_pool.stop_condition_met():
        gene_pool.evolve()
        print(f"Iteration {gene_pool.current_iteration}/{MAX_ITERATIONS} completed - Best fitness: {gene_pool.genes[0].evaluate(gene_pool.inputs, gene_pool.targets):.5f}")
